...
class User:
    ...
    def update(self):
        response = json.loads(
            requests.get(f"https://api.scratch.mit.edu/users/{self.username}/").text
        )
    ...
    def is_following(self, user):
        return requests.get(
            f"https://following-check.1tim.repl.co/api/{self.username}/?following={user}"
        ).json()["following"]

    def is_followed_by(self, user):
        return requests.get(
            f"https://following-check.1tim.repl.co/api/{user}/?following={self.username}"
        ).json()["following"]
    ...
    def set_featured(self, project_id, *, label=""):
        requests.put(
            ...
            cookies=self._cookies,
            data=json.dumps(
                {"featured_project": int(project_id), "featured_project_label": label}
            ),
        )
    ...
    def followers_over_time(self, *, segment=1, range=30):
        return requests.get(
            f"https://scratchdb.lefty.one/v3/user/graph/{self.username}/followers?segment={segment}&range={range}"
        )

    def forum_counts(self):
        try:
            return requests.get(
                f"https://scratchdb.lefty.one/v3/forum/user/info/{self.username}"
            ).json()["counts"]
        except Exception:
    ...
    def forum_signature_history(self):
        return requests.get(
            f"https://scratchdb.lefty.one/v3/forum/user/history/{self.username}"
        ).json()
    ...
    def forum_posts(self, *, page=0, order="newest"):
        try:
            data = requests.get(
                f"https://scratchdb.lefty.one/v3/forum/user/posts/{self.username}/{page}?o={order}"
            ).json()
            return_data = []
            for o in data:
