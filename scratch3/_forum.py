...
class ForumTopic:
    ...
    def update(self):
        topic = requests.get(
            f"https://scratchdb.lefty.one/v3/forum/topic/info/{self.id}"
        ).json()
    ...
    def activity(self):
        return requests.get(
            f"https://scratchdb.lefty.one/v3/forum/topic/history/{self.id}"
        ).json()

    def post_count_by_user(self, username):
        data = requests.get(
            f"https://scratchdb.lefty.one/v3/forum/topic/graph/{self.id}/{username}?segment=1&range=1"
        ).json()
        if len(data) == 0:
            return 0
        else:
    ...
    def posts(self, *, page=0, order="oldest"):
        data = requests.get(
            f"https://scratchdb.lefty.one/v3/forum/topic/posts/{self.id}/{page}?o={order}"
        ).json()
        return_data = []
        for o in data:
    ...
    def first_post(self):
        o = requests.get(
            f"https://scratchdb.lefty.one/v3/forum/topic/posts/{self.id}/0?o=oldest"
        ).json()[0]
        a = ForumPost(id=o["id"], _session=self._session)
        a._update_from_dict(o)
        return a
...
class ForumPost:
    ...
    def update(self):
        post = requests.get(
            f"https://scratchdb.lefty.one/v3/forum/post/info/{self.id}"
        ).json()
    ...
    def ocular_reactions(self):
        return requests.get(
            f"https://my-ocular.jeffalo.net/api/reactions/{self.id}"
        ).json()
