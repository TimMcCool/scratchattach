...
class Project(PartialProject):
    ...
    def studios(self, *, limit=40, offset=0):
        data = requests.get(
            f"https://api.scratch.mit.edu/users/{self.author}/projects/{self.id}/studios?limit={limit}&offset={offset}"
        ).json()
    ...
    def studios(self):
        return requests.get(
            f"https://api.scratch.mit.edu/users/{self.author}/projects/{self.id}/studios"
        ).json()
