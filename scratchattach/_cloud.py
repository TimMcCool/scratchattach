...
class TwCloudConnection(_CloudMixin):
    def _connect(self, *, cloud_host):
        ...
        try:
            ...
            self.websocket.connect(cloud_host, enable_multithread=True)
        except Exception:
            raise (_exceptions.ConnectionError)
...
def get_cloud_logs(
    project_id,
    *,
    filter_by_var_named=None,
    limit=25,
    offset=0,
    log_url="https://clouddata.scratch.mit.edu/logs",
):
    try:
        response = json.loads(
            requests.get(
                f"{log_url}?projectid={project_id}&limit={limit}&offset={offset}"
            ).text
        )
