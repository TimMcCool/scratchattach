...
class EducatorAlert:
    ...
    def from_json(cls, data: dict[str, Any], _session: Optional[session.Session] = None) -> Self:
        ...
        object_id = admin_action.get("object_id")
        ...
        if "project_title" in extra_data:
            target_object = project.Project(id=object_id, title=extra_data["project_title"], _session=_session)
            ...
            if comment_type == 0:
                ...
                warnings.warn(
                    f'The parser was not able to recognise the "comment_type" of {comment_type} in the alert JSON response.\nFull response: \n{pprint.pformat(data)}.\n\nPlease draft an issue on github: https://github.com/TimMcCool/scratchattach/issues, providing this whole error message. This will allow us to implement an incomplete part of this parser'
                )
            target_object = comment.Comment(
                id=object_id, content=content, source=comment_source_type, source_id=comment_obj_id, _session=_session
            )
        elif "gallery_title" in extra_data:
            target_object = studio.Studio(id=object_id, title=extra_data["gallery_title"], _session=_session)
