...
class CloudActivity(BaseSiteComponent[Union[typed_dicts.CloudActivityDict, typed_dicts.CloudLogActivityDict]]):
    ...
    def __eq__(self, activity2):
        return (
            self.user == activity2.user
            and self.type == activity2.type
            and (self.timestamp == activity2.timestamp)
            and (self.value == activity2.value)
            and (self.name == activity2.name)
        )

    def _update_from_data(self, data: Union[typed_dicts.CloudActivityDict, typed_dicts.CloudLogActivityDict]) -> bool:

        def is_cloud_log_activity(
            activity: Union[typed_dicts.CloudActivityDict, typed_dicts.CloudLogActivityDict],
        ) -> TypeGuard[typed_dicts.CloudLogActivityDict]:
            return "verb" in activity

        def is_cloud_activity(
            activity: Union[typed_dicts.CloudActivityDict, typed_dicts.CloudLogActivityDict],
        ) -> TypeGuard[typed_dicts.CloudActivityDict]:
    ...
    def load_log_data(self):
        if self.cloud is None:
            print(
                "Warning: There aren't cloud logs available for this cloud, therefore the user and exact timestamp can't be loaded"
            )
        elif isinstance(self.cloud, _base.LogCloud):
