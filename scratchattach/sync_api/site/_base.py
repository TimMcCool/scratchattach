...
class BaseSiteComponent(ABC, Generic[D]):
    ...
    def update(self):
        ...
        """
        with self._session.http_session.request(
...
def api_iterative(
    ...
    *,
    ...
    def fetch(off: int, lim: int) -> list[F] | None:
        """
        Performs a single API request
        """
        with session.http_session.get(
            f"{url}?limit={lim}&offset={off}{add_params}", shared_http.options().headers(_headers).cookies(cookies).value
        ) as response:
            resp = cast(list[F], response.json())
            if not resp:
                return None
    ...
    return api_iterative_data(fetch, limit, offset, max_req_limit=max_req_limit, unpack=True)
