...
def get_cloud_status() -> CloudStatus:
    with requests.no_error_handling():
        try:
            ...
            try:
                data = resp.json()
                return CloudStatus(True, resp.content, uptime=data["uptime"], load=data["load"], redis=data["redis"])
...
def check_email(email: str) -> bool:
    ...
    """
    data = requests.get(
        f"https://scratch.mit.edu/accounts/check_email/",
        params={
            "email": email,
        },
    ).json()[0]
...
def translate(language: str | Languages | Language, text: str = "hello"):
    ...
    response_json = requests.get(
        f"https://translate-service.scratch.mit.edu/translate?language={lang.code}&text={text}"
    ).json()
...
def text2speech(
    text: str = "hello", voice_name: str | TTSVoices | TTSVoice = "female", language: str | Language | Languages = "en-US"
):
    """
    ...
    if voice.name == "kitten":
        text = " ".join(("meow" for word in text.split(" ") if word.strip()))
