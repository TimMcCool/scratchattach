...
@dataclass
class Studio(BaseSiteComponent[StudioDict]):
    """
    ...
    def _update_from_data(self, studio: StudioDict):
    ...
    def comment_replies(self, *, comment_id, limit=40, offset=0) -> list[comment.Comment]:
        response = commons.api_iterative(
            f"https://api.scratch.mit.edu/studios/{self.id}/comments/{comment_id}/replies",
            limit=limit,
            offset=offset,
            add_params=f"&cachebust={random.randint(0, 9999)}",
        )
    ...
    def comment_by_id(self, comment_id):
        r = requests.get(f"https://api.scratch.mit.edu/studios/{self.id}/comments/{comment_id}", timeout=10).json()
        if r is None:
    ...
    def projects(self, limit=40, offset=0) -> list[project.Project]:
        ...
        """
        response = commons.api_iterative(f"https://api.scratch.mit.edu/studios/{self.id}/projects", limit=limit, offset=offset)
    ...
    def curators(self, limit=40, offset=0) -> list[user.User]:
        ...
        """
        response = commons.api_iterative(f"https://api.scratch.mit.edu/studios/{self.id}/curators", limit=limit, offset=offset)
    ...
    def add_project(self, project_id):
        ...
        return requests.post(
            f"https://api.scratch.mit.edu/studios/{self.id}/project/{project_id}", headers=self._headers, timeout=10
        ).json()
    ...
    def managers(self, limit=40, offset=0):
        ...
        """
        response = commons.api_iterative(f"https://api.scratch.mit.edu/studios/{self.id}/managers", limit=limit, offset=offset)
        return commons.parse_object_list(response, user.User, self._session, "username")
