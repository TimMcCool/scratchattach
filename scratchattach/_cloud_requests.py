...
class CloudRequests:
    ...
    def __init__(
        ...
        *,
        ...
        if _log_url != "https://clouddata.scratch.mit.edu/logs":
            print(
                "Warning: Log URL isn't the URL of Scratch's clouddata logs. Don't use the _log_url parameter unless you know what you are doing."
            )
        if _packet_length > 245:
            print(
                "Warning: The packet length was set to a value higher than default (245). Your project most likely won't work on Scratch."
            )
    ...
    def request(self, function=None, *, enabled=True, name=None, thread=False):
        def inner(function):
            if thread:
                ...
                print(
                    "Warning: Running individual requests in threads increases CPU usage"
                )
    ...
    def call_request(self, request_id, req_obj, arguments):
        ...
        try:
            ...
            if req_obj["thread"]:
                ...
                self._parse_output(
                    "Error: Check the Python console", request, req_obj, request_id
                )
        if not self.respond_in_thread:
            self.cloud_events.resume()
    ...
    def edit_request(
        self, name, *, enabled=None, new_name=None, new_function=None, thread=None
    ):
    ...
    def _respond(
        self, request_id, response, limit, *, force_reconnect=False, validation=2222
    ):
    ...
    def _run(self, events, data_from_websocket=False):
        ...
        if data_from_websocket:
            ...
            self.last_data = _cloud.get_cloud_logs(
                self.project_id, limit=100, log_url=self.log_url
            )
        self.last_timestamp = 0

        if not data_from_websocket:
            data = _cloud.get_cloud_logs(
                self.project_id, limit=100, log_url=self.log_url
            )
            if data == []:
                pass
                ...
                for activity in data:
                    if activity["timestamp"] > self.last_timestamp:
                        ...
                        if request not in self.requests:
                            ...
                            if req_obj["thread"]:
                                Thread(
                                    target=self.call_request,
                                    args=(request_id, req_obj, arguments),
                                ).start()
                            else:
...
class TwCloudRequests(CloudRequests):
    def __init__(
        ...
        *,
        ...
        if _packet_length > 98800:
            print(
                "Warning: The packet length was set to a value higher than TurboWarp's default (98800)."
            )
        self.used_cloud_vars = used_cloud_vars
        self.connection = cloud_connection
        self.project_id = cloud_connection.project_id
    ...
    def run(self, thread=False, data_from_websocket=True):
        ...
        if thread:
            thread = Thread(
                target=self._run, args=[events], kwargs={"data_from_websocket": True}
            )
            thread.start()
        else:
